...
//
// Only RGBA8 is currently supported as an input format. Alpha is honoured:
// pixels below the alpha threshold given to GifBegin() (GIF_ALPHA_THRESHOLD by
// default) are stored with the reserved transparency index. Pass 0 to ignore
// alpha and quantise every pixel as an opaque colour.
//
...
// GIF only has binary transparency: a pixel is either fully opaque or fully
// transparent. Source pixels whose alpha is below this value are written with
// the reserved transparency index, everything else is quantised as an opaque
// colour. Pass an alpha threshold of 0 to GifBegin() to ignore alpha
// completely (the historical behaviour of this file).
#ifndef GIF_ALPHA_THRESHOLD
#define GIF_ALPHA_THRESHOLD 128
#endif
...
// Keeps only the pixels that will actually be visible in the GIF (i.e. the ones
// that are not fully transparent) so that transparent pixels do not waste
// palette entries on colours nobody will ever see.
int GifPickOpaquePixels( const uint8_t* image, uint8_t* out, int numPixels, int alphaThreshold )
{
    int numOpaque = 0;
...
void GifMakePalette( const uint8_t* lastFrame, const uint8_t* nextFrame, uint32_t width, uint32_t height, int bitDepth, bool buildForDither, GifPalette* pPal, int alphaThreshold = GIF_ALPHA_THRESHOLD )
{
    (void)lastFrame; // alpha aware frames are always written in full, so there is no delta to compare against
    memset(pPal, 0, sizeof(GifPalette));
    ...
    if(alphaThreshold > 0)
        numPixels = GifPickOpaquePixels(nextFrame, destroyableImage, numPixels, alphaThreshold);
    else
    {
...
void GifDitherImage( const uint8_t* lastFrame, const uint8_t* nextFrame, uint8_t* outFrame, uint32_t width, uint32_t height, GifPalette* pPal, int alphaThreshold = GIF_ALPHA_THRESHOLD )
{
    int numPixels = (int)(width * height);
    ...
    for( uint32_t yy=0; yy<height; ++yy )
    {
        for( uint32_t xx=0; xx<width; ++xx )
        {
            ...
            // fully transparent pixels use the reserved transparency index
            if( nextFrame[4*(yy*width+xx)+3] < alphaThreshold )
            {
                nextPix[0] = nextPix[1] = nextPix[2] = 0;
                nextPix[3] = 0;
                continue;
            }
...
void GifThresholdImage( const uint8_t* lastFrame, const uint8_t* nextFrame, uint8_t* outFrame, uint32_t width, uint32_t height, GifPalette* pPal, int alphaThreshold = GIF_ALPHA_THRESHOLD )
{
    ...
    for( uint32_t ii=0; ii<numPixels; ++ii )
    {
        ...
        {
            outFrame[0] = outFrame[1] = outFrame[2] = 0;
...
void GifWriteLzwImage(FILE* f, uint8_t* image, uint32_t left, uint32_t top,  uint32_t width, uint32_t height, uint32_t delay, GifPalette* pPal, int disposalMethod = 1)
{
    ...
    fputc(0xf9, f);
    fputc(0x04, f);
    // packed field: disposal method in bits 4-2, transparent colour flag in bit 0.
    // Disposal 1 ("leave in place") is only correct for frames that rely on the
    // previous one; frames that carry real transparency must use disposal 2
    // ("restore to background") or the transparent areas keep showing the
    // previous frame.
    fputc((disposalMethod << 2) | 0x01, f);
    fputc(delay & 0xff, f);
...
typedef struct
{
    FILE* f;
    uint8_t* oldImage;
    // Source alpha below this value becomes transparent. 0 disables alpha
...
bool GifWriteFrame( GifWriter* writer, const uint8_t* image, uint32_t width, uint32_t height, uint32_t delay, int bitDepth = 8 )
{
    ...
    // the frames carry no real transparency: with disposal 2 every frame is
    // drawn on a cleared canvas, so relying on the previous frame would make
    // those pixels disappear.
    const uint8_t* oldImage = (!useAlpha && !writer->firstFrame)? writer->oldImage : NULL;
    writer->firstFrame = false;

    // Frames with transparency must restore the background before they are
    // drawn, otherwise the transparent areas show the previous frame instead
    // of what is behind the GIF.
    const int disposalMethod = useAlpha? 2 : 1;

    GifPalette pal;
    GifMakePalette((dither? NULL : oldImage), image, width, height, bitDepth, dither, &pal, writer->alphaThreshold);

    if(dither)
        GifDitherImage(oldImage, image, writer->oldImage, width, height, &pal, writer->alphaThreshold);
    else
        GifThresholdImage(oldImage, image, writer->oldImage, width, height, &pal, writer->alphaThreshold);
